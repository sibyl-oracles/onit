...
class TestIntermediateFold:
    """A turn that ends in a tool call folds its narration; the answer never does."""

    def test_folded_turn_is_not_persisted_to_history(self, chat_ui, capsys):
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_token("The ninja PATH issue again. Fixing it now.")
        chat_ui.stream_fold("The ninja PATH issue again.")
        chat_ui.stream_end()
        assert [m.role for m in chat_ui.messages] == []

    def test_fold_leaves_a_step_marker(self, chat_ui, capsys):
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_token("Checking the failing kernel compile.")
        chat_ui.stream_fold("Checking the failing kernel compile.")
        out = capsys.readouterr().out
        assert "Checking the failing kernel compile." in out  # the step marker
        assert "·" in out

    def test_fold_resets_stream_state_for_the_next_turn(self, chat_ui, capsys):
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_token("Step one narration.")
        chat_ui.stream_fold("Step one narration.")
        assert chat_ui._stream_header_printed is False
        assert chat_ui._streaming_content == ""
        # The next turn streams and closes as a normal block.
        chat_ui.stream_start()
        chat_ui.stream_token("The real answer.")
        chat_ui.stream_end()
        assert [m.role for m in chat_ui.messages] == ["assistant"]

    def test_fold_without_visible_stream_is_a_silent_reset(self, chat_ui, capsys):
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_fold()  # no header, no tokens: nothing to erase or mark
        out = capsys.readouterr().out
        assert "·" not in out
        chat_ui.stream_end()
        assert [m.role for m in chat_ui.messages] == []

    def test_oversized_block_is_kept_but_still_unpersisted(self, chat_ui, capsys, monkeypatch):
        monkeypatch.setattr(chat_ui, "_stream_rows", lambda: 40)
        erased = []
        monkeypatch.setattr(chat_ui, "_erase_streamed_block", lambda: erased.append(1))
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_token("A long narration block that fills the screen. " * 10)
        chat_ui.stream_fold("A long narration block.")
        assert erased == []  # too tall to retract safely
        chat_ui.stream_end()
        assert [m.role for m in chat_ui.messages] == []  # still kept out of history

    def test_trackable_block_is_erased(self, chat_ui, capsys, monkeypatch):
        monkeypatch.setattr(chat_ui, "_stream_rows", lambda: 3)
        erased = []
        monkeypatch.setattr(chat_ui, "_erase_streamed_block", lambda: erased.append(1))
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_token("Short narration.")
        chat_ui.stream_fold("Short narration.")
        assert erased == [1]

    def test_show_intermediate_restores_the_old_behavior(self, chat_ui, capsys):
        chat_ui.show_intermediate = True
        chat_ui.console = Console(file=sys.stdout, width=120)
        chat_ui.stream_start()
        chat_ui.stream_token("Narration the old way.")
        chat_ui.stream_fold("Narration the old way.")  # must be a no-op
        chat_ui.stream_end()
        roles = [m.role for m in chat_ui.messages]
        assert roles == ["assistant"]  # persisted, as before the fold existed
        out = capsys.readouterr().out
        assert "·" not in out

    def test_cursor_row_is_none_without_a_tty(self, chat_ui):
        # capsys stdout is not a tty: no cursor query, no hang, None.
        assert chat_ui._cursor_row() is None
        assert chat_ui._stream_rows() is None
